...
- `docs/issues/calendar.md`, and the date-type decision in it, hard-blocked
  `date-picker.md`. **Discharged** — `src/elements/calendar.rs` exists, and the
  date-type question is answered by `src/date.rs`: three integers and Hinnant's
  civil-day arithmetic, so no `chrono` / `time` / `jiff` reaches the public API.
  The grid takes its visible month in as well as out (`set_visible_month`
  paired with `CalendarEvent::MonthChanged`), which is the one requirement
  `date-picker.md` imposes on it.
